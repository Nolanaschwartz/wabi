...
**Tech Stack:** TypeScript, Node.js 20, discord.js v14, Next.js 15 (App Router), Tailwind CSS, Vercel AI SDK, Mem0 (self-hosted), Qdrant (self-hosted), **Langfuse** (self-hosted), OpenAI-compatible LLM endpoint (GPT-4o for PoC; local/open model as the destination), PostgreSQL (Prisma ORM), Stripe, Docker, Express (webhooks), lucia-auth (Discord OAuth)
