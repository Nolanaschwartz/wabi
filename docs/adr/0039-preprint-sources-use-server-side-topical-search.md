...
- **Two live-API assumptions to verify at implementation** (not from memory): OSF `filter[q]` does usable full-text search over preprint title/abstract; Europe PMC `SRC:PPR` + field-tag + `cursorMark` syntax. If OSF `filter[q]` is weak, PsyArXiv may fall back to EPMC's partial coverage or stay windowed *in isolation* — which would not reintroduce the medRxiv hang.
  ...
  **Resolved during implementation (2026-06-19):** Europe PMC `/search` confirmed (`format=json`, `resultType=core`, `SRC:PPR`, `cursorMark`/`nextCursorMark`, relevance default). OSF has **no** `filter[q]` on `/v2/preprints/` — a live probe returns `HTTP 400 "'q' is not a valid field for this endpoint"`. The fallback taken is **`filter[description][icontains]=<phrase>`** (verified: returns hundreds of all-history matches), issued once per core phrase and merged (OSF AND-s multiple `filter[]`, so synonyms must be separate requests). PsyArXiv stays a first-class topical source — no windowing, no EPMC dependency.
