...
- **Ground truth is independent *intent*, never the prompt under test.** Each step's dataset is labeled against a short, prompt-agnostic rubric of what the output *should* be — not by reading the step's current prompt and labeling to match it. Labeling-by-the-prompt would only measure instruction-following: a prompt encoding the wrong policy scores 100%, and every prompt edit invalidates the labels. Intent-based labels survive prompt edits and are the only thing that lets an eval *drive* a prompt toward correctness rather than merely confirm the model can read. (This is what makes the eval usable when we do not yet trust a step's policy — which is the starting condition for the `gate`.)
...
- **No change to the agent or the bot.** Eval scripts call the step functions directly and resolve providers the same lazy way the worker does (CLAUDE.md "resolve config lazily"); nothing in the production path moves. Submission to the strategy-admin API (ADR-0012/0034) is untouched. In particular, prompts-under-test stay **in code** (not Langfuse prompt management) — experiment runs are tagged with the git SHA — so the always-on worker's hot path never gains a remote prompt fetch for the sake of evals.
- **Eval traces go to a separate Langfuse project** (own key pair), so synthetic experiment runs never contaminate the production score analytics or blur the online↔offline boundary above.
