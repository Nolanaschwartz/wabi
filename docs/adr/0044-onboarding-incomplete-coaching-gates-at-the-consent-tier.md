...
- **Rollout caveat (2026-06-29):** the `onboardingCompletedAt` migration is added nullable with **no backfill**, so on deploy *every* pre-existing consented User is un-onboarded at once — momentarily the opposite of "transient." This is accepted **only because the product is pre-launch** (effectively no existing user base). Once there is a real installed base, a launch that ships this gate MUST backfill `onboardingCompletedAt` for already-consented Users (or move the gate after the classifier per the rejected alternative above), or it would drop active subscribers to tripwire-only screening and cut their coaching until they re-onboard. Surfaced by the slice-01–06 code review.
