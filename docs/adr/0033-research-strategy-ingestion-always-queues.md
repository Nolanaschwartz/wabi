...
- **The worker never reads or writes the bot's coaching stores directly.** Candidate ingest still flows over HTTP only — `POST /admin/strategies/ingest` to submit and `GET /admin/strategies/seen` to check a source-ID ledger (`ProcessedSource`) for cross-run idempotency (`AdminGuard` on both). The bot remains the single writer of Postgres `StrategyDraft` and `wabi_strategies` and writes the ledger itself at ingest, so all trust/safety/dedup logic stays single-sourced (ADR-0012/0004).
  - **Amended by ADR-0034** — the original "the worker keeps no DB credentials" claim is superseded. The worker is now an always-on NestJS service with `DATABASE_URL`, but its database footprint is scoped to its **own** research tables (`ResearchConfig`/`ResearchTopic`/`ResearchRun`) plus the pg-boss queue it owns. "No DB creds" becomes **"no *user-data* access"**: it still never reads or writes `User`, any inner-state model, or `StrategyDraft`, and candidates still reach the strategy library only over HTTP through the bot's trust gate. The trust gate remains the single path into the library.
- **Public data only (ADR-0002).** The worker has no access to user data, Redis session buffers, or personal memory. *(Amended by ADR-0034: the worker is now a separate **always-on** service — see ADR-0034 — but remains isolated from user data and does not run on the bot's crisis-safe DM hot path, ADR-0019/0020.)*
