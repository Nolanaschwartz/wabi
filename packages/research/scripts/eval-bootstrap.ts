...
async function toRow(p: Paper, topic: string, bucket: Bucket): Promise<DatasetRow> {
  // Topic-aware gate; bootstrap only needs the keep/reject verdict, so a no-tracer genObj (runs the
  // model, emits no span) is enough.
  const r = await relevanceGate(makeResearchGenerateObject(), p.abstract, topic);
